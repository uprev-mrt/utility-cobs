...
TEST(CobFifo_Test, pushframe )
{
  cobs_fifo_t cobFifo;
  int nextLen;
  cobs_fifo_init(&cobFifo,128);

  cobs_fifo_push_frame(&cobFifo, testFrame0, FRAME_0_LEN);  //encoding adds one byte
...
TEST(CobFifo_Test, popframe )
{
  ...
  cobs_fifo_push_frame(&cobFifo, testFrame0, FRAME_0_LEN);  //encoding adds one byte
  cobs_fifo_pop_frame(&cobFifo, testComp0, FRAME_0_LEN);
...
TEST(CobFifo_Test, multiframes )
{
  ...
  cobs_fifo_push_frame(&cobFifo, testFrame0, FRAME_0_LEN);  //encoding adds one byte
  cobs_fifo_push_frame(&cobFifo, testFrame1, FRAME_1_LEN);

  nextLen = cobs_fifo_get_next_len(&cobFifo);
  ...
  decodedLen = cobs_fifo_pop_frame(&cobFifo, testComp0, FRAME_0_LEN);
  ...
  decodedLen = cobs_fifo_pop_frame(&cobFifo, testComp1, FRAME_1_LEN);
